...
private:
  ...
  // OPT-8: Replaced std::unordered_map with std::vector for faster linear stack scans
  ...
  void recompute_accumulators(const Board& board, int ply, bool white_side, bool black_side) {
    static NNUE::NNUEParameters p = NNUE::get_parameters();
    
    if (white_side) {
      std::memcpy(m_accum_friend_white[ply], p.friend_ft_bias, 256 * sizeof(int16_t));
      std::memcpy(m_accum_enemy_white[ply], p.enemy_ft_bias, 256 * sizeof(int16_t));
    }
    if (black_side) {
      std::memcpy(m_accum_friend_black[ply], p.friend_ft_bias, 256 * sizeof(int16_t));
      std::memcpy(m_accum_enemy_black[ply], p.enemy_ft_bias, 256 * sizeof(int16_t));
    }
    
    int king_white = board.kingSq(Color::WHITE).index();
    int king_black = board.kingSq(Color::BLACK).index();
    
    for (int sq_idx = 0; sq_idx < 64; sq_idx++) {
      Square sq = Square(sq_idx);
      Piece pc = board.at(sq);
      if (pc != Piece::NONE) {
        int piece_type = static_cast<int>(pc.type()) + 1;
        int color = (pc.color() == Color::WHITE) ? 0 : 1;
        
        if (white_side) {
          int idx_us = NNUE::friend_idx(piece_type, color, sq_idx, king_white, 0);
          NNUE::accum_add(m_accum_friend_white[ply], p.friend_ft_weights, idx_us);
          
          int idx_them = NNUE::enemy_idx(piece_type, color, sq_idx, king_black, 0);
          if (idx_them >= 0) {
            NNUE::accum_add(m_accum_enemy_white[ply], p.enemy_ft_weights, idx_them);
          }
        }
        
        if (black_side) {
          int idx_us = NNUE::friend_idx(piece_type, color, sq_idx, king_black, 1);
          NNUE::accum_add(m_accum_friend_black[ply], p.friend_ft_weights, idx_us);
          
          int idx_them = NNUE::enemy_idx(piece_type, color, sq_idx, king_white, 1);
          if (idx_them >= 0) {
            NNUE::accum_add(m_accum_enemy_black[ply], p.enemy_ft_weights, idx_them);
          }
        }
      }
    }
  ...
  void update_accumulators(const Board& board, Move move, int ply) {
    static NNUE::NNUEParameters p = NNUE::get_parameters();
    
    std::memcpy(m_accum_friend_white[ply + 1], m_accum_friend_white[ply], 256 * sizeof(int16_t));
    std::memcpy(m_accum_friend_black[ply + 1], m_accum_friend_black[ply], 256 * sizeof(int16_t));
    std::memcpy(m_accum_enemy_white[ply + 1], m_accum_enemy_white[ply], 256 * sizeof(int16_t));
    std::memcpy(m_accum_enemy_black[ply + 1], m_accum_enemy_black[ply], 256 * sizeof(int16_t));
    
    Piece pc = board.at(move.from());
    int piece_type = static_cast<int>(pc.type()) + 1;
    int color = (pc.color() == Color::WHITE) ? 0 : 1;
    ...
    // 1. Subtract moved piece from source square
    int idx_us_w = NNUE::friend_idx(piece_type, color, from_idx, king_white, 0);
    NNUE::accum_sub(m_accum_friend_white[ply + 1], p.friend_ft_weights, idx_us_w);
    int idx_them_w = NNUE::enemy_idx(piece_type, color, from_idx, king_black, 0);
    if (idx_them_w >= 0) NNUE::accum_sub(m_accum_enemy_white[ply + 1], p.enemy_ft_weights, idx_them_w);
    
    int idx_us_b = NNUE::friend_idx(piece_type, color, from_idx, king_black, 1);
    NNUE::accum_sub(m_accum_friend_black[ply + 1], p.friend_ft_weights, idx_us_b);
    int idx_them_b = NNUE::enemy_idx(piece_type, color, from_idx, king_white, 1);
    if (idx_them_b >= 0) NNUE::accum_sub(m_accum_enemy_black[ply + 1], p.enemy_ft_weights, idx_them_b);
    
    // 2. Add moved/promoted piece to destination square
    int final_type = piece_type;
    if (move.typeOf() == Move::PROMOTION) {
      final_type = static_cast<int>(move.promotionType()) + 1;
    }
    
    int idx_to_us_w = NNUE::friend_idx(final_type, color, to_idx, king_white, 0);
    NNUE::accum_add(m_accum_friend_white[ply + 1], p.friend_ft_weights, idx_to_us_w);
    int idx_to_them_w = NNUE::enemy_idx(final_type, color, to_idx, king_black, 0);
    if (idx_to_them_w >= 0) NNUE::accum_add(m_accum_enemy_white[ply + 1], p.enemy_ft_weights, idx_to_them_w);
    
    int idx_to_us_b = NNUE::friend_idx(final_type, color, to_idx, king_black, 1);
    NNUE::accum_add(m_accum_friend_black[ply + 1], p.friend_ft_weights, idx_to_us_b);
    int idx_to_them_b = NNUE::enemy_idx(final_type, color, to_idx, king_white, 1);
    if (idx_to_them_b >= 0) NNUE::accum_add(m_accum_enemy_black[ply + 1], p.enemy_ft_weights, idx_to_them_b);
    
    // 3. Handle captures
    if (is_capture && move.typeOf() != Move::ENPASSANT) {
      int cap_type = static_cast<int>(captured.type()) + 1;
      int cap_color = (captured.color() == Color::WHITE) ? 0 : 1;
      
      int idx_cap_us_w = NNUE::friend_idx(cap_type, cap_color, to_idx, king_white, 0);
      NNUE::accum_sub(m_accum_friend_white[ply + 1], p.friend_ft_weights, idx_cap_us_w);
      int idx_cap_them_w = NNUE::enemy_idx(cap_type, cap_color, to_idx, king_black, 0);
      if (idx_cap_them_w >= 0) NNUE::accum_sub(m_accum_enemy_white[ply + 1], p.enemy_ft_weights, idx_cap_them_w);
      
      int idx_cap_us_b = NNUE::friend_idx(cap_type, cap_color, to_idx, king_black, 1);
      NNUE::accum_sub(m_accum_friend_black[ply + 1], p.friend_ft_weights, idx_cap_us_b);
      int idx_cap_them_b = NNUE::enemy_idx(cap_type, cap_color, to_idx, king_white, 1);
      if (idx_cap_them_b >= 0) NNUE::accum_sub(m_accum_enemy_black[ply + 1], p.enemy_ft_weights, idx_cap_them_b);
    }
    else if (move.typeOf() == Move::ENPASSANT) {
      Square cap_sq = Square(move.to().file(), move.from().rank());
      int cap_color = 1 - color;
      
      int idx_cap_us_w = NNUE::friend_idx(1, cap_color, cap_sq.index(), king_white, 0);
      NNUE::accum_sub(m_accum_friend_white[ply + 1], p.friend_ft_weights, idx_cap_us_w);
      int idx_cap_them_w = NNUE::enemy_idx(1, cap_color, cap_sq.index(), king_black, 0);
      if (idx_cap_them_w >= 0) NNUE::accum_sub(m_accum_enemy_white[ply + 1], p.enemy_ft_weights, idx_cap_them_w);
      
      int idx_cap_us_b = NNUE::friend_idx(1, cap_color, cap_sq.index(), king_black, 1);
      NNUE::accum_sub(m_accum_friend_black[ply + 1], p.friend_ft_weights, idx_cap_us_b);
      int idx_cap_them_b = NNUE::enemy_idx(1, cap_color, cap_sq.index(), king_white, 1);
      if (idx_cap_them_b >= 0) NNUE::accum_sub(m_accum_enemy_black[ply + 1], p.enemy_ft_weights, idx_cap_them_b);
    }
  }
...
public:
  ...
  int evaluate(const Board &board, int depth, int ply) {
    ...
    int side_pov = (side == Color::WHITE) ? 0 : 1;
    int oriented_king_us = NNUE::orient(side_pov, board.kingSq(side).index());
    int oriented_king_them = NNUE::orient(!side_pov, board.kingSq(~side).index());
    
    int32_t raw_score;
    if (side == Color::WHITE) {
      raw_score = NNUE::evaluate(oriented_king_us, oriented_king_them, 
                                 m_accum_friend_white[ply], m_accum_enemy_white[ply]);
    } else {
      raw_score = NNUE::evaluate(oriented_king_us, oriented_king_them, 
                                 m_accum_friend_black[ply], m_accum_enemy_black[ply]);
    }
    
    int score = (raw_score - 4341) / 96;
    return score;
  }

  int quiescence(Board &board, int alpha, int beta, int qdepth, int ply) {
    m_node_count++;
    ...
    if (qdepth > 4 || reason != GameResultReason::NONE) {
      return evaluate(board, 0, ply);
    }

    int stand_pat = evaluate(board, 0, ply);
    if (stand_pat >= beta)
      return beta;
    if (stand_pat > alpha)
      alpha = stand_pat;
    ...
    for (const auto &sc : scored_captures) {
      ...
      Color mover_color = board.at(move.from()).color();
      bool is_king_move = (board.at(move.from()).type() == PieceType::KING) || (move.typeOf() == Move::CASTLING);
      if (is_king_move) {
        update_accumulators(board, move, ply);
        board.makeMove(move);
        recompute_accumulators(board, ply + 1, true, true);
      } else {
        update_accumulators(board, move, ply);
        board.makeMove(move);
      }
      
      int score = -quiescence(board, -beta, -alpha, qdepth + 1, ply + 1);
  ...
  int alpha_beta(Board &board, int depth, int alpha, int beta, int extensions,
                 std::vector<uint64_t> &search_history, int ply) {
    m_node_count++;
    if ((m_node_count & 4095) == 0) {
      auto now = std::chrono::steady_clock::now();
      double elapsed =
    ...
    // OPT-8: Repetition check using std::vector linear scan
    int rep_count = 0;
    ...
    auto [reason, result] = board.isGameOver();
    if (reason != GameResultReason::NONE) {
      return evaluate(board, depth, ply);
    }
    ...
    int static_eval = evaluate(board, 0, ply);

    int original_alpha = alpha;

    // BUG-3: Null Move Pruning (NMP) zero-window search
    int R = (depth >= 6) ? 3 : 2;
    if (depth >= R + 1 && !in_check && has_major_pieces(board)) {
      ...
      int null_score = -alpha_beta(board, depth - 1 - R, -beta, -beta + 1,
                                   extensions, search_history, ply + 1);
    ...
    for (int i = 0; i < (int)moves.size(); i++) {
      ...
      // OPT-3: Principal Variation Search (PVS) with LMR
      if (new_depth >= 3 && i >= 3 && is_quiet && !gives_check && !in_check) {
        ...
        score = -alpha_beta(board, new_depth - reduction, -(alpha + 1), -alpha,
                            new_ext, search_history, ply + 1);
        if (score > alpha) {
          score = -alpha_beta(board, new_depth, -beta, -alpha, new_ext,
                              search_history, ply + 1);
        }
      } else if (i > 0) {
        // Zero-window search for non-PV moves
        score = -alpha_beta(board, new_depth, -(alpha + 1), -alpha, new_ext, search_history, ply + 1);
        if (score > alpha && score < beta) {
          // Research with full window
          score = -alpha_beta(board, new_depth, -beta, -alpha, new_ext, search_history, ply + 1);
        }
      } else {
        // PV move gets full window
        score = -alpha_beta(board, new_depth, -beta, -alpha, new_ext, search_history, ply + 1);
      }
  ...
  Move search_best_move(Board board, int max_depth,
    ...
    // Initialize root accumulators (ply = 0)
    recompute_accumulators(board, 0, true, true);
    ...
    Move best_move = Move::NO_MOVE;
    int last_score = -9999999;

    try {
      for (int depth = 1; depth <= max_depth; depth++) {
        // OPT-1: Aspiration Windows search logic
        int alpha = -9999999;
        int beta = 9999999;
        int delta = 50;

        if (depth >= 3 && last_score != -9999999) {
        ...
        while (true) {
          ...
          for (const auto &move : moves) {
            ...
            bool is_king_move = (board.at(move.from()).type() == PieceType::KING) || (move.typeOf() == Move::CASTLING);
            if (is_king_move) {
              update_accumulators(board, move, 0);
              board.makeMove(move);
              recompute_accumulators(board, 1, true, true);
            } else {
              update_accumulators(board, move, 0);
              board.makeMove(move);
            }
            ...
            int score =
                -alpha_beta(board, depth - 1, -beta, -alpha, 0, search_history, 1);
            board.unmakeMove(move);
  ...
  int solve(py::array_t<int8_t> obs, py::array_t<int8_t> mask, int tb_action) {
    ...
    m_game_history.push_back(cur_hash);
    ...
    throw std::runtime_error("No valid move found from Opening Book or Search Engine!");
  }

  bool test_incremental_vs_recompute_moves(const std::vector<std::string> &moves_uci) {
    static NNUE::NNUEParameters p = NNUE::get_parameters();
    Board board;
    recompute_accumulators(board, 0, true, true);
    
    for (size_t i = 0; i < moves_uci.size(); i++) {
      Move move;
      try {
        move = uci::uciToMove(board, moves_uci[i]);
      } catch (...) {
        std::cout << "Invalid UCI move: " << moves_uci[i] << std::endl;
        return false;
      }
      
      bool is_king_move = (board.at(move.from()).type() == PieceType::KING) || (move.typeOf() == Move::CASTLING);
      
      // Compute incremental at ply = 1
      update_accumulators(board, move, 0);
      
      // Make move
      board.makeMove(move);
      
      // Save incremental accumulators
      alignas(32) int16_t inc_friend_white[256];
      alignas(32) int16_t inc_friend_black[256];
      alignas(32) int16_t inc_enemy_white[256];
      alignas(32) int16_t inc_enemy_black[256];
      std::memcpy(inc_friend_white, m_accum_friend_white[1], 256 * sizeof(int16_t));
      std::memcpy(inc_friend_black, m_accum_friend_black[1], 256 * sizeof(int16_t));
      std::memcpy(inc_enemy_white, m_accum_enemy_white[1], 256 * sizeof(int16_t));
      std::memcpy(inc_enemy_black, m_accum_enemy_black[1], 256 * sizeof(int16_t));
      
      // Compute recomputed accumulators at ply = 1
      recompute_accumulators(board, 1, true, true);
      
      // If it's NOT a king move, incremental MUST match recomputed exactly!
      if (!is_king_move) {
        for (int k = 0; k < 256; k++) {
          if (inc_friend_white[k] != m_accum_friend_white[1][k] ||
              inc_friend_black[k] != m_accum_friend_black[1][k] ||
              inc_enemy_white[k] != m_accum_enemy_white[1][k] ||
              inc_enemy_black[k] != m_accum_enemy_black[1][k]) {
            std::cout << "Mismatch at move " << moves_uci[i] << " index " << k << std::endl;
            std::cout << "Friend White: incremental=" << inc_friend_white[k] << ", recompute=" << m_accum_friend_white[1][k] << std::endl;
            std::cout << "Friend Black: incremental=" << inc_friend_black[k] << ", recompute=" << m_accum_friend_black[1][k] << std::endl;
            std::cout << "Enemy White: incremental=" << inc_enemy_white[k] << ", recompute=" << m_accum_enemy_white[1][k] << std::endl;
            std::cout << "Enemy Black: incremental=" << inc_enemy_black[k] << ", recompute=" << m_accum_enemy_black[1][k] << std::endl;
            return false;
          }
        }
      }
      
      // Shift ply 1 to ply 0 for the next move
      std::memcpy(m_accum_friend_white[0], m_accum_friend_white[1], 256 * sizeof(int16_t));
      std::memcpy(m_accum_friend_black[0], m_accum_friend_black[1], 256 * sizeof(int16_t));
      std::memcpy(m_accum_enemy_white[0], m_accum_enemy_white[1], 256 * sizeof(int16_t));
      std::memcpy(m_accum_enemy_black[0], m_accum_enemy_black[1], 256 * sizeof(int16_t));
    }
    return true;
  }

  int evaluate_nnue_direct(const std::string &fen) {
    Board board(fen);
    recompute_accumulators(board, 0, true, true);
    ...
    if (side == Color::WHITE) {
      return NNUE::evaluate(oriented_king_us, oriented_king_them, 
                            m_accum_friend_white[0], m_accum_enemy_white[0]);
    } else {
      return NNUE::evaluate(oriented_king_us, oriented_king_them, 
                            m_accum_friend_black[0], m_accum_enemy_black[0]);
    }
  }
};
...
PYBIND11_MODULE(chess_engine_d8_han, m) {
  m.doc() = "D8 C++ Chess Engine Module (Thread-Safe Instance Version)";

  py::class_<SearchEngine>(m, "SearchEngineD8")
      .def(py::init<>())
      ...
      .def("evaluate_nnue_direct", &SearchEngine::evaluate_nnue_direct, "Evaluate board using NNUE directly")
      .def("test_incremental_vs_recompute_moves", &SearchEngine::test_incremental_vs_recompute_moves, "Test incremental updates against full recomputation")
