...
def get_enemy_features(board: chess.Board, is_white_pov: bool, option):
    king_sq = orient(is_white_pov, board.king(is_white_pov))
    enemy_indices = []
    for sq, p in board.piece_map().items():
...
def run_forward(path, board, option):
    ...
    enemy_indices = get_enemy_features(board, board.turn == chess.WHITE, option)
...
def test():
    ...
    for option in [1, 2]:
        ...
        for name, b in positions.items():
            score = run_forward('weights/nn.nnue', b, option)
