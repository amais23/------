...
def verify(path):
    ...
    for k in range(4):
        ...
        # L1 bias: 16 int32s (64 bytes)
        l1_bias = np.frombuffer(data[start+4 : start+4+64], dtype=np.int32)
        print(f"L1 bias: min={l1_bias.min()}, max={l1_bias.max()}, mean={l1_bias.mean():.2f}")
        print(f"  values: {l1_bias}")
        
        # L1 weights: 16 * 1024 int8s (16384 bytes)
        l1_weights = np.frombuffer(data[start+68 : start+68+16384], dtype=np.int8).reshape(16, 1024)
        print(f"L1 weights: min={l1_weights.min()}, max={l1_weights.max()}, mean={l1_weights.mean():.4f}, std={l1_weights.std():.4f}")
        print(f"  first few weights of row 0: {l1_weights[0, :16]}")
