...
def trace(path, board, name):
    ...
    print(f"\n===== Tracing: {name} =====")
    print(f"acc_us mean: {acc_us.mean():.2f}, std: {acc_us.std():.2f}")
    print(f"acc_them mean: {acc_them.mean():.2f}, std: {acc_them.std():.2f}")
    
    # Division factor sweep or 127? Let's use 127 (which is floor mode) and print out_l1
    acc_us_scaled = acc_us // 127
    acc_them_scaled = acc_them // 127
    
    in_l1 = np.concatenate([
        np.clip(acc_us_scaled, 0, 127).astype(np.int8),
        np.clip(acc_them_scaled, 0, 127).astype(np.int8)
    ])
    
    print(f"in_l1 non-zero count: {np.count_nonzero(in_l1)} / 512")
    ...
    print(f"  first 8 values: {list(out_l1[:8])}")
