...
def evaluate_relative_board(board: chess.Board, depth: int = 0) -> int:
    ...
    # B. 動態國王安全評估 (King Safety: Pawn Shield & Open Files)
    # 1. 我方國王安全 (White King)
    white_king_sq = board.king(chess.WHITE)
    white_king_file = white_king_sq & 7
    # 執短易位 (Kingside)
    if white_king_file >= 5: # f1, g1, h1
        # f2 (sq 13)
        f2_p = board.piece_at(13)
        if not (f2_p and f2_p.piece_type == chess.PAWN and f2_p.color == chess.WHITE):
            f3_p = board.piece_at(21)
            score -= 8 if (f3_p and f3_p.piece_type == chess.PAWN and f3_p.color == chess.WHITE) else 20
        # g2 (sq 14)
        g2_p = board.piece_at(14)
        if not (g2_p and g2_p.piece_type == chess.PAWN and g2_p.color == chess.WHITE):
            g3_p = board.piece_at(22)
            score -= 12 if (g3_p and g3_p.piece_type == chess.PAWN and g3_p.color == chess.WHITE) else 30
        # h2 (sq 15)
        h2_p = board.piece_at(15)
        if not (h2_p and h2_p.piece_type == chess.PAWN and h2_p.color == chess.WHITE):
            h3_p = board.piece_at(23)
            score -= 8 if (h3_p and h3_p.piece_type == chess.PAWN and h3_p.color == chess.WHITE) else 20
        # 開直列懲罰
        for f in (5, 6, 7):
            white_pawns = board.pieces(chess.PAWN, chess.WHITE) & chess.BB_FILES[f]
            if not white_pawns:
                score -= 15
                if not (board.pieces(chess.PAWN, chess.BLACK) & chess.BB_FILES[f]):
                    score -= 15
    # 執長易位 (Queenside)
    elif white_king_file <= 2: # a1, b1, c1
        # a2 (sq 8)
        a2_p = board.piece_at(8)
        if not (a2_p and a2_p.piece_type == chess.PAWN and a2_p.color == chess.WHITE):
            a3_p = board.piece_at(16)
            score -= 8 if (a3_p and a3_p.piece_type == chess.PAWN and a3_p.color == chess.WHITE) else 20
        # b2 (sq 9)
        b2_p = board.piece_at(9)
        if not (b2_p and b2_p.piece_type == chess.PAWN and b2_p.color == chess.WHITE):
            b3_p = board.piece_at(17)
            score -= 12 if (b3_p and b3_p.piece_type == chess.PAWN and b3_p.color == chess.WHITE) else 30
        # c2 (sq 10)
        c2_p = board.piece_at(10)
        if not (c2_p and c2_p.piece_type == chess.PAWN and c2_p.color == chess.WHITE):
            c3_p = board.piece_at(18)
            score -= 8 if (c3_p and c3_p.piece_type == chess.PAWN and c3_p.color == chess.WHITE) else 20
        # 開直列懲罰
        for f in (0, 1, 2):
            white_pawns = board.pieces(chess.PAWN, chess.WHITE) & chess.BB_FILES[f]
            if not white_pawns:
    ...
    # 2. 對手國王暴露 (Black King, 獎勵我方)
    black_king_sq = board.king(chess.BLACK)
    black_king_file = black_king_sq & 7
    # 對手短易位
    if black_king_file >= 5: # f8, g8, h8
        # f7 (sq 53)
        f7_p = board.piece_at(53)
        if not (f7_p and f7_p.piece_type == chess.PAWN and f7_p.color == chess.BLACK):
            f6_p = board.piece_at(45)
            score += 8 if (f6_p and f6_p.piece_type == chess.PAWN and f6_p.color == chess.BLACK) else 20
        # g7 (sq 54)
        g7_p = board.piece_at(54)
        if not (g7_p and g7_p.piece_type == chess.PAWN and g7_p.color == chess.BLACK):
            g6_p = board.piece_at(46)
            score += 12 if (g6_p and g6_p.piece_type == chess.PAWN and g6_p.color == chess.BLACK) else 30
        # h7 (sq 55)
        h7_p = board.piece_at(55)
        if not (h7_p and h7_p.piece_type == chess.PAWN and h7_p.color == chess.BLACK):
            h6_p = board.piece_at(47)
            score += 8 if (h6_p and h6_p.piece_type == chess.PAWN and h6_p.color == chess.BLACK) else 20
        # 對手開直列獎勵我方
        for f in (5, 6, 7):
            black_pawns = board.pieces(chess.PAWN, chess.BLACK) & chess.BB_FILES[f]
            if not black_pawns:
                score += 15
                if not (board.pieces(chess.PAWN, chess.WHITE) & chess.BB_FILES[f]):
                    score += 15
    # 對手長易位
    elif black_king_file <= 2: # a8, b8, c8
        # a7 (sq 48)
        a7_p = board.piece_at(48)
        if not (a7_p and a7_p.piece_type == chess.PAWN and a7_p.color == chess.BLACK):
            a6_p = board.piece_at(40)
            score += 8 if (a6_p and a6_p.piece_type == chess.PAWN and a6_p.color == chess.BLACK) else 20
        # b7 (sq 49)
        b7_p = board.piece_at(49)
        if not (b7_p and b7_p.piece_type == chess.PAWN and b7_p.color == chess.BLACK):
            b6_p = board.piece_at(41)
            score += 12 if (b6_p and b6_p.piece_type == chess.PAWN and b6_p.color == chess.BLACK) else 30
        # c7 (sq 50)
        c7_p = board.piece_at(50)
        if not (c7_p and c7_p.piece_type == chess.PAWN and c7_p.color == chess.BLACK):
            c6_p = board.piece_at(42)
            score += 8 if (c6_p and c6_p.piece_type == chess.PAWN and c6_p.color == chess.BLACK) else 20
        # 對手開直列獎勵我方
        for f in (0, 1, 2):
            black_pawns = board.pieces(chess.PAWN, chess.BLACK) & chess.BB_FILES[f]
            if not black_pawns:
