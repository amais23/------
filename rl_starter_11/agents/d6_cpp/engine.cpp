...
0x9bd395339cdbf4a7ULL, 0x05a7e8a57db91b77ULL, 0xb5889c6e15630a75ULL, 0x4a750a09ce9573f7ULL,
...
inline void tt_store(uint64_t key, int depth, int score, TTFlag flag, Move best_move) {
...
static const int16_t* PIECE_PST[6] = {
...
uint64_t hash_ep_square(const Board& board) {
    ...
    if (ep != Square::NO_SQ) {
        ...
        if (file > 0) adjacent_pawns |= (1ULL << (target_rank * 8 + file - 1));
        if (file < 7) adjacent_pawns |= (1ULL << (target_rank * 8 + file + 1));
...
uint64_t polyglot_hash(const Board& board) {
    ...
    if (cr.has(Color::WHITE, Board::CastlingRights::Side::KING_SIDE))  hash ^= POLYGLOT_RANDOM_ARRAY[768];
    if (cr.has(Color::WHITE, Board::CastlingRights::Side::QUEEN_SIDE)) hash ^= POLYGLOT_RANDOM_ARRAY[768 + 1];
    if (cr.has(Color::BLACK, Board::CastlingRights::Side::KING_SIDE))  hash ^= POLYGLOT_RANDOM_ARRAY[768 + 2];
    if (cr.has(Color::BLACK, Board::CastlingRights::Side::QUEEN_SIDE)) hash ^= POLYGLOT_RANDOM_ARRAY[768 + 3];
...
inline bool has_major_pieces(const Board& board) {
    for (Color c : {Color::WHITE, Color::BLACK}) {
        if (board.pieces(PieceType::QUEEN, c) ||
            board.pieces(PieceType::ROOK, c) ||
...
int evaluate(const Board& board, int depth) {
    ...
    // Game Phase
    int knight_count = board.pieces(PieceType::KNIGHT, Color::WHITE).count() + board.pieces(PieceType::KNIGHT, Color::BLACK).count();
    int bishop_count = board.pieces(PieceType::BISHOP, Color::WHITE).count() + board.pieces(PieceType::BISHOP, Color::BLACK).count();
    int rook_count   = board.pieces(PieceType::ROOK, Color::WHITE).count() + board.pieces(PieceType::ROOK, Color::BLACK).count();
    int queen_count  = board.pieces(PieceType::QUEEN, Color::WHITE).count() + board.pieces(PieceType::QUEEN, Color::BLACK).count();
    ...
    // Bishop Pair
    if (board.pieces(PieceType::BISHOP, Color::WHITE).count() == 2) score += 50;
    if (board.pieces(PieceType::BISHOP, Color::BLACK).count() == 2) score -= 50;

    // King Castled
    Square ksq_w = board.kingSq(Color::WHITE);
    if (ksq_w == Square("c1") || ksq_w == Square("g1") || ksq_w == Square("b1")) score += 30;

    Square ksq_b = board.kingSq(Color::BLACK);
    if (ksq_b == Square("c8") || ksq_b == Square("g8") || ksq_b == Square("b8")) score -= 30;
...
int quiescence(Board& board, int alpha, int beta, int qdepth) {
    ...
    if ((g_node_count & 4095) == 0) {
        ...
        double elapsed = std::chrono::duration<double>(now - g_search_start).count();
        if (elapsed > g_time_limit) throw std::runtime_error("timeout");
    }
    ...
    for (const auto& move : captures) {
        ...
        int score = 10000 + PIECE_VAL[static_cast<int>(victim)] - (PIECE_VAL[static_cast<int>(attacker)] / 10);
    ...
    for (const auto& sc : scored_captures) {
        ...
        // Delta Pruning
        if (stand_pat + victim_val + 200 < alpha && move.typeOf() != Move::PROMOTION) {
            continue;
        }
...
int alpha_beta(Board& board, int depth, int alpha, int beta, int extensions,
    ...
    if (tt && tt->depth >= depth) {
        ...
        if (tt->flag == TT_BETA  && tt->score >= beta)  return tt->score;
        if (tt->flag == TT_ALPHA && tt->score <= alpha) return tt->score;
    }
    ...
    if (depth >= R + 1 && !in_check && has_major_pieces(board)) {
        board.makeNullMove();
        int null_score = -alpha_beta(board, depth - 1 - R, -beta, -alpha, extensions, search_history);
    ...
    for (int i = 0; i < (int)moves.size(); i++) {
        ...
        if (new_depth >= 3 && i >= 3 && is_quiet && !gives_check && !in_check) {
            ...
            score = -alpha_beta(board, new_depth - reduction, -(alpha + 1), -alpha, new_ext, search_history);
            if (score > alpha) {
                score = -alpha_beta(board, new_depth, -beta, -alpha, new_ext, search_history);
            }
        } else {
            score = -alpha_beta(board, new_depth, -beta, -alpha, new_ext, search_history);
        }
    ...
    search_history[key]--;
    if (search_history[key] == 0) search_history.erase(key);
...
void order_moves(Movelist& moves, const Board& board, Move tt_move, int depth) {
    ...
    for (const auto& move : moves) {
        ...
        if (move == tt_move) {
            ...
            PieceType victim = board.getCapturing<PieceType>(move);
            PieceType attacker = board.at<PieceType>(move.from());
            score = 10000 + PIECE_VAL[static_cast<int>(victim)] - (PIECE_VAL[static_cast<int>(attacker)] / 10);
        } else {
            if (depth < 64) {
                if (move == g_killers[depth][0]) {
                    score = 9000;
...
// ═══════════════════════════════════════════
Move search_best_move(Board& board, int max_depth, std::unordered_map<uint64_t, int>& search_history) {
    ...
    if (phase >= 8)      g_time_limit = 1.5;
    else if (phase >= 4) g_time_limit = 0.8;
    else                 g_time_limit = 0.4;
    ...
    if (best_move == Move::NO_MOVE) {
        ...
        if (fallback.size() > 0) best_move = fallback[0];
    }
...
void load_book(const std::string& path) {
    ...
    while (f.good()) {
        uint8_t buf[16];
        if (!f.read(reinterpret_cast<char*>(buf), 16)) break;
    ...
    std::sort(g_book_entries.begin(), g_book_entries.end(),
...
Board relative_to_absolute(const Board& rel_board) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (p != Piece::NONE) {
            ...
            char c_char = ' ';
            if (type == PieceType::PAWN) c_char = 'P';
            else if (type == PieceType::KNIGHT) c_char = 'N';
            else if (type == PieceType::BISHOP) c_char = 'B';
            else if (type == PieceType::ROOK) c_char = 'R';
            else if (type == PieceType::QUEEN) c_char = 'Q';
            else if (type == PieceType::KING) c_char = 'K';
    ...
    for (int r = 0; r < 8; r++) {
        ...
        if (empty_count > 0) placement += std::to_string(empty_count);
        if (r < 7) placement += "/";
    }
    
    std::string castling_str = "";
    auto rel_cr = rel_board.castlingRights();
    if (rel_cr.has(Color::BLACK, Board::CastlingRights::Side::KING_SIDE))  castling_str += "K";
    if (rel_cr.has(Color::BLACK, Board::CastlingRights::Side::QUEEN_SIDE)) castling_str += "Q";
    if (rel_cr.has(Color::WHITE, Board::CastlingRights::Side::KING_SIDE))  castling_str += "k";
    if (rel_cr.has(Color::WHITE, Board::CastlingRights::Side::QUEEN_SIDE)) castling_str += "q";
    if (castling_str.empty()) castling_str = "-";
    ...
    if (ep != Square::NO_SQ) {
        ...
        ep_str = std::string(1, 'a' + (abs_ep_idx % 8)) + std::to_string((abs_ep_idx / 8) + 1);
    }
...
Move decode_polyglot_move(uint16_t raw_move, const Board& board) {
    ...
    if (promo_val > 0) {
        ...
        if (promo_val == 1) promo_type = PieceType::KNIGHT;
        else if (promo_val == 2) promo_type = PieceType::BISHOP;
        else if (promo_val == 3) promo_type = PieceType::ROOK;
        else promo_type = PieceType::QUEEN;
        
        return Move::make<Move::PROMOTION>(source, target, promo_type);
    }
    
    if (pt == PieceType::KING && std::abs((to_val % 8) - (from_val % 8)) == 2) {
        Square rook_target = Square(to_val > from_val ? File::FILE_H : File::FILE_A, source.rank());
...
Move probe_book(const Board& board, bool is_black) {
    if (g_book_entries.empty()) return Move::NO_MOVE;

    Board query_board = is_black ? relative_to_absolute(board) : board;
    uint64_t poly_key = polyglot_hash(query_board);

    auto it = std::lower_bound(g_book_entries.begin(), g_book_entries.end(),
                               poly_key, [](const BookEntry& e, uint64_t k) {
                                   return e.key < k;
                               });
    ...
    if (matches.empty()) return Move::NO_MOVE;
    ...
    if (is_black && abs_move != Move::NO_MOVE) {
        ...
        if (abs_move.typeOf() == Move::CASTLING) {
            ...
            return Move::make<Move::PROMOTION>(from_rel, to_rel, abs_move.promotionType());
...
int get_move_plane(Move move) {
    ...
    bool is_knight = (std::abs(dx) + std::abs(dy) == 3) && (std::abs(dx) >= 1 && std::abs(dx) <= 2);
    if (is_knight) {
    ...
    if (move.typeOf() == Move::PROMOTION) {
        ...
        if (promo != PieceType::QUEEN) {
            ...
            int piece_idx = (promo == PieceType::KNIGHT) ? 0 : ((promo == PieceType::BISHOP) ? 1 : 2);
...
int fallback_random(const py::detail::unchecked_reference<int8_t, 1>& mask) {
    ...
    for (int i = 0; i < 4672; i++) {
        if (mask(i) == 1) legals.push_back(i);
    }
...
void engine_init(const std::string& book_path) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        for (int file_check = std::max(0, f - 1); file_check <= std::min(7, f + 1); file_check++) {
...
int engine_solve(py::array_t<int8_t> obs, py::array_t<int8_t> mask, int tb_action) {
    try {
    ...
    try {
        ...
        if (board.sideToMove() == Color::BLACK) {
            ...
            if (m.typeOf() == Move::CASTLING) {
                ...
                rel_move = Move::make<Move::PROMOTION>(from_rel, to_rel, m.promotionType());
...
std::pair<uint64_t, std::string> test_book_info(const std::string& fen, bool is_black) {
    Board board(fen);
    Board query_board = is_black ? relative_to_absolute(board) : board;
    uint64_t hash = polyglot_hash(query_board);
    Move book_move = probe_book(board, is_black);
    std::string move_uci = (book_move != Move::NO_MOVE) ? uci::moveToUci(book_move) : "";
    return {hash, move_uci};
}

PYBIND11_MODULE(chess_engine, m) {
    m.doc() = "D6 C++ Chess Search Engine";
    m.def("init", &engine_init, py::arg("book_path") = "", "Initialize engine");
    m.def("new_game", &engine_new_game, "Reset game-specific state");
    m.def("solve", &engine_solve, py::arg("observation"), py::arg("action_mask"), py::arg("tb_action") = -1, "Find best action");
